...
export default function Sidebar({ karyawan, isAdmin }) {
    ...
    // --- PERBAIKAN: Buat variabel boolean untuk 'Admin Non-Penilaian' ---
    const isNonAssessmentAdmin = karyawan?.tipe_akun === 'Admin Non-Penilaian';

    return (
        <aside className={`bg-white shadow-md flex flex-col p-4 transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
            
            {/* ... (Blok header dan tombol expand/collapse) ... */}
            ...
            <nav className="flex-grow space-y-2">
                ...
                {isNonAssessmentAdmin && (
                     <NavLink href="/dashboard/my-assessments" isSidebarOpen={isSidebarOpen} text="Behavioral" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-600 group-hover:text-teal-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>} />
                )}

                {!isNonAssessmentAdmin && (
                <NavLink href="/dashboard" isSidebarOpen={isSidebarOpen} text="KPI" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-600 group-hover:text-teal-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6a2 2 0 012-2h12a2 2 0 012 2v12a2 2 0 01-2 2H6a2 2 0 01-2-2V6z" /></svg>} />
                )}

                {!isNonAssessmentAdmin && (
                    <NavLink href="/dashboard/behavioral" isSidebarOpen={isSidebarOpen} text="Behavioral" icon={
                ...
                {/* --- MENU BARU: Struktur Organisasi --- */}
                <NavLink 
                    href="/dashboard/struktur-organisasi" 
                    isSidebarOpen={isSidebarOpen} 
                    text="Struktur Organisasi" 
                    icon={
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-600 group-hover:text-teal-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
